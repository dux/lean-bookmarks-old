/*
 ...
 - app/assets/admin/js/admin_bootstrap3.min.js
 - app/assets/admin/js/document_on.js.coffee
 - app/assets/admin/js/pjax.coffee
 - app/assets/admin/js/to-markdown.js
 - app/assets/admin/js/toastr.coffee
 - app/assets/admin/index.root.coffee
*/
...
/* File: app/assets/admin/js/document_on.js */
// Generated by CoffeeScript 1.7.1
(function() {
  this.DocumentOn = {
    func: {},
    refresh: function(el, nn) {
      return alert('Find widget and refresh it');
    },
    click: function(el) {
      var func, nn, ret, _i, _len, _ref;
      nn = el[0].nodeName;
      if (el.attr('onclick')) {
        return;
      }
      ret = [];
      _ref = ['confirm', 'form', 'href'];
    ...
    keyup: function() {
      return $.throttle('enable_if', 300, function() {
        return $('*[enable_if]').each(function() {
          var target, val;
          target = $(this);
          if (val = target.attr('enable_if')) {
            return target[0].disabled = !eval(val);
          }
        });
      });
    }
        ...
        if (aa = form.attr('api-action')) {
          $.post("/api/" + aa, form.serialize(), function(response) {
            ...
            if (done = form.attr('done')) {
              if (done === 'refresh') {
                return Pjax.load(location.href.replace(/https?:\/\/[^\/]+/, ''));
              } else if (done === 'redirect' || done === 'admin') {
                return Pjax.load('/admin' + response.path);
              } else {
                return eval(done);
              }
            }
  ...
  $(function() {
    ...
    $doc.on('click', function(event) {
      return DocumentOn.click($(event.target));
    });
    $doc.on('keyup', function(event) {
      return DocumentOn.keyup($(event.target));
    });
    $doc.on('paste', function(event) {
      return DocumentOn.keyup($(event.target));
    });
    return $doc.bind('pjax:get', function(event) {
      return DocumentOn.keyup($(event.target));
    });
  });
...
/* File: app/assets/admin/js/pjax.js */
// Generated by CoffeeScript 1.7.1
(function() {
  this.Pjax = {
    skip_on: [],
    ...
    init: function(full_page) {
      ...
      if (window.history && window.history.pushState) {
        ...
        setTimeout(function() {
          return Pjax.load_is_safe = true;
        }, 1000);
        window.onpopstate = function(event, href) {
          if (event.state && event.state.body) {
            ...
            Pjax.load(levent.state.href, {
              history: true
            });
          }
          return $(document).trigger('pjax:get');
        };
        return $(window).trigger('page:change');
      }
    },
    skip: function() {
...
/* File: app/assets/admin/js/toastr.js */
// Generated by CoffeeScript 1.7.1
(function() {
  this.Info = {
    ok: function(msg) {
...
(function() {
  $.extend({
    ...
    throttle: function(id, ms, func) {
      window.throttle || (window.throttle = {});
      if (window.throttle[id]) {
        clearTimeout(window.throttle[id]);
      }
      return window.throttle[id] = setTimeout(function() {
  ...
  this.Rails = {
    "delete": function(what, id, url) {
      if (!confirm('Shure ?')) {
        return false;
      }
      return $.post("/api/" + what + "/" + id + "/destroy", '', function(response) {
        Info.auto(response);
        if (!response.error) {
          url || (url = "/admin/" + what);
          return Pjax.load(url);
        }
      });
    },
    update: function(what, id, opts) {
      return $.post("/api/" + what + "/" + id + "/update", opts, function(response) {
        return Info.auto(response);
      });
    },
    api: function(url, opts, func) {
      return $.post("/api/" + url, opts, function(response) {
        Info.auto(response);
        if (func) {
          return func();
        }
  ...
  $(function() {
    $.cachedScript('http://api.filepicker.io/v1/filepicker.js', function() {
      return filepicker.setKey('Ad2YNAorRgmFCY1XphLPgz');
    });
    Pjax.init('#full_page');
    return Pjax.skip(function(el) {
      return !/\/admin\//.test(el);
    });
  });
