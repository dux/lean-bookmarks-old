...
(function() {
  this.Api = {
    post: function(method, opts) {
      var _disable_button, i, key, len, ref, ref1;
      if (opts == null) {
        opts = {};
      }
      if (opts.done) {
      ...
      if ((ref = opts.params) != null ? ref.getAttribute : void 0) {
        opts.params = $(opts.params).serializeHash();
      }
      opts.silent || (opts.silent = false);
      if (opts.disable) {
        $(opts.disable).disable();
      }
      ref1 = Object.keys(opts);
      ...
      return $.post(method, opts.params, function(ret) {
        if (!opts['silent']) {
          Info.auto(ret);
        }
...
(function() {
  window.TopModal = {
    render: function(title, data, no_overlay) {
      var overlay;
      overlay = no_overlay ? '' : '<div id="top_modal_overlay" style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(100, 100, 100, 0.5) ;z-index:10;" onclick="TopModal.close();"></div>';
    ...
    load: function(title, url, no_overlay) {
      TopModal.render(title, '...', no_overlay);
      return $.get(url, function(data) {
        return $('#top_modal_data').html(data);
      });
    },
    app: {
      select_bucket: function(id, no_overlay) {
        if (no_overlay == null) {
          no_overlay = false;
        }
        return TopModal.load('Select bucket', "/bucket/select?id=" + id, no_overlay);
      }
    }
  };
...
(function() {
  ...
  window.App = {
    ...
    link_drag: function(ev, id) {
      ev.dataTransfer.setData("link_id", id);
      return TopModal.app.select_bucket(0, 'no_overlay');
    },
    link_drop: function(ev, id) {
      var link_id;
      link_id = ev.dataTransfer.getData("link_id");
      TopModal.close();
      return Api.send("links/" + link_id + "/move", {
        bucket_id: id
      });
    }
  };
